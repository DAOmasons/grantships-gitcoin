export const ROUND_DATA: Record<
  string,
  {
    reportLink: string;
    roundLink: string;
  }
> = {
  //regen coordination
  '0x2377bfa0777c385f4d3fdee97d1a65228f836da67ea6f0db96235870ac5cdc82': {
    reportLink:
      'https://gov.gitcoin.co/t/ai-impactqf-regen-coordination-global-gg23-retrospective/20385/2',
    roundLink: 'https://explorer.gitcoin.co/#/round/42220/31',
  },
  // Good Dollar
  '0xd18608a33ecfd6b9c08a6c8fd99fd934199dff54ae53515e159b195b2cce770c': {
    reportLink:
      'https://medium.com/gooddollar/gooddollar-builders-first-qf-round-results-978cbc67eed0',
    roundLink: 'https://explorer.gitcoin.co/#/round/42220/29',
  },
  // Token Engineering Commons
  '0xae3add0277ad57036fb34714bdd96872dc971915636e3cafab9aa08e2632b8cf': {
    reportLink:
      'https://docs.google.com/document/d/1zUUWCrVRUxhuqxFVY2IsFyZpQafOHDOh/edit?usp=sharing&ouid=103939713320425009736&rtpof=true&sd=true',
    roundLink:
      'https://easyretropgf.xyz/token-engineering-retractive-funding-round-2024-25',
  },
  // Regen Rio de Janeiro
  '0x22ed83fde1924b615d904ccb2259c3ab42e72ec0870242399cda94743503682e': {
    reportLink:
      'https://various-option-5d8.notion.site/Regen-Rio-1cd38e08e9f680a8a089ee953dbaed7f?pvs=4',
    roundLink: 'https://explorer.gitcoin.co/#/round/42220/30',
  },
  // Gitcoin Grants Garden
  '0x781ce54b73730667e7390caa0e52d4d585b574f274f780c4e0bb96a18a996c30': {
    reportLink: 'https://hackmd.io/yE1Vc--MTcWSoMQzuESWNA',
    roundLink:
      'https://app.gardens.fund/gardens/10/0x1eba7a6a72c894026cd654ac5cdcf83a46445b08/0xd3345828914b740fddd1b8ae4f4d2ce03d1e0960',
  },
  // web3 for universities
  '0x559f71a3559444c0e12205b095cbecd784199ab4caa271fcddf67b293318eb0f': {
    reportLink:
      'https://gov.gitcoin.co/t/web3-for-universities-community-round-full-round-report/20351',
    roundLink: 'https://explorer.gitcoin.co/#/round/42220/27',
  },
} as const;
